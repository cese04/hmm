...
class HMM(object):
    ...
    def fit(self, X, max_iter=30):
        ...
        for it in range(max_iter):
            ...
            self.pi = np.sum((alphas[n][0] * betas[n][0]) / P[n]
                             for n in range(N)) / N
            ...
            for n in range(N):
                ...
                den1 += (alphas[n][:-1] * betas[n][:-1]
                         ).sum(axis=0, keepdims=True).T / P[n]
                den2 += (alphas[n] * betas[n]).sum(axis=0,
                                                   keepdims=True).T / P[n]
    ...
    def get_state_sequence(self, x):
        ...
        for t in range(1, T):
            for j in range(self.M):
                delta[t, j] = np.max(
                    delta[t - 1] * self.A[:, j] * self.B[j, x[t]])
...
def fit_coin():
    ...
    L = hmm.log_likelihood_multi(X).sum()
    print("LL with real parameters: ", L)

    print("Best sequence for: ", X[0])
