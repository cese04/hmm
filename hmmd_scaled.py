        ...
        for it in range(max_iter):
            ...
            for n in range(N):
                ...
                for t in range(T - 2, -1, -1):
                    beta[t] = self.A.dot(
                        self.B[:, x[t + 1]] * beta[t + 1]) / scale[t + 1]
                betas.append(beta)

            cost = np.sum(logP)
            costs.append(cost)

            self.pi = np.sum((alphas[n][0] * betas[n][0]
                              for n in range(N))) / N
            ...
            for n in range(N):
                ...
                den1 += (alphas[n][:-1] * betas[n][:-1]
                         ).sum(axis=0, keepdims=True).T
                den2 += (alphas[n] * betas[n]).sum(axis=0, keepdims=True).T

                for i in range(self.M):
                    for j in range(self.M):
                        for t in range(T - 1):
                            a_num[i, j] += alphas[n][t, i] * betas[n][t + 1, j] * \
                                self.A[i, j] * self.B[j, x[t + 1]] / \
                                scales[n][t + 1]
